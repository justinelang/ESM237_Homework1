...
clim$heat = ifelse(clim$TMAX <= 90, 0, 1)
clim.wfrdata = clim %>% group_by(year(date)) %>% summarize(ndayfr=sum(heat))
clim.wfrdata$dt = unique(year(date))

ggplot(clim.wfrdata, aes(dt, ndayfr))+geom_col(position = "stack", fill="red")+labs(x = "Year", y="Number of Extreme Heat Days") + theme_minimal()
```
...
ggplot(clim.pk, aes(dt, hotday))+geom_point(col = "red")+labs(y="Hottest Day in the Year") + theme_minimal()
```
...
``` {r return period}
clim$flood = ifelse(clim$PRCP >= 1.5, 1, 0)
...
clim.fldata = clim %>% group_by(year(date)) %>% summarize(ndayfl=sum(flood))
clim.fldata$dt = unique(year(date))

ggplot(clim.fldata, aes(dt, ndayfl))+geom_col(position = "stack", fill="blue")+labs(x = "Year", y="Number of Extreme Precipitation Days") + theme_minimal()
```

```{r}
library(Kendall)
MannKendall(clim.wfrdata$ndayfr)
MannKendall(clim.fldata$ndayfl)
```
