...
clim = read_csv(here("gaylordbay.csv")) %>% 
...
fillrow = which(is.na(clim$TMAX) | clim$TMAX < 10)
...
# early portion
res_early=lm(tmin~dt, data=subset(clim.mwy, clim.mwy$dt %in% c(2010:2015)))
summary(res_early)
confint(res_early,"dt", level=0.90)
ggplot(subset(clim.mwy, clim.mwy$dt %in% c(2010:2015)), aes(x=dt, y=tmin)) + stat_summary(fun.y="mean", geom="point", col="red", size=4)+theme(axis.text=element_text(size=14, face="bold"), axis.title=element_text(size=14, face="bold")) + geom_smooth(method="lm")

# last decade
res_late=lm(tmin~dt, data=subset(clim.mwy, clim.mwy$dt %in% c(2016:2023)))
summary(res_late)
confint(res_late,"dt", level=0.90)
ggplot(subset(clim.mwy, clim.mwy$dt %in% c(2016:2023)), aes(x=dt, y=tmin)) + stat_summary(fun.y="mean", geom="point", col="red", size=4)+theme(axis.text=element_text(size=14, face="bold"), axis.title=element_text(size=14, face="bold")) + geom_smooth(method="lm")
...
t.test(subset(clim.mwy$tmin, clim.mwy$dt %in% 2010:2015), subset(clim.mwy$tmin, clim.mwy$dt %in% 2010:2015))

wilcox.test(subset(clim.mwy$tmin, clim.mwy$dt %in% 2016:2023), subset(clim.mwy$tmin, clim.mwy$dt %in% 2016:2023))
